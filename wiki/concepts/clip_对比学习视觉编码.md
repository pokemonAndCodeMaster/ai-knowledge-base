...
**核心源码解剖 (Numpy 伪代码)**：
```python
# I_f, T_f: 分别经过 Image Encoder 和 Text Encoder 提取的特征
I_f = image_encoder(I) #[n, d_i]
T_f = text_encoder(T) #[n, d_t]

# 投射层: 映射到统一的多模态维度 d_e，并做 L2 归一化
I_e = l2_normalize(np.dot(I_f, W_i), axis=1)
T_e = l2_normalize(np.dot(T_f, W_t), axis=1)

# 计算相似度 logits矩阵 (n x n)
logits = np.dot(I_e, T_e.T) * np.exp(t)

# 对称的对比损失：因为正样本都在对角线上，所以 ground truth 标签就是 [0, 1, 2... n-1]
labels = np.arange(n)
...
**零样本预测 (Zero-shot Prediction) 的魔法**：
CLIP 不像传统分类器那样预测一个具体的“类别 ID”，而是采用了 **Prompt Template**。
比如在推理时，要分类猫狗。模型会将词汇填入模板 `"A photo of a {object}"`，得到 `"A photo of a cat"` 和 `"A photo of a dog"`，提取这两个句子的特征，然后算它们与图像特征的余弦相似度，相似度最高的即为分类结果。这让它摆脱了预定类别的束缚。

**局限性**：由于分母中需要求和 Batch 内所有的负样本，这要求在分布式训练时，所有的 GPU 必须将彼此的特征矩阵通过 `all-gather` 进行全局同步（Global Normalization）。这导致计算 and 通信开销随着 Batch Size 的增大呈二次方爆炸（$O(B^2)$）。
