...
平台不提供“管理者/运营人员”手工切换，也不为不同岗位复制两套页面。用户登录后进入同一条业务信息架构，权限自动作用于四个层级：

| 层级 | 权限效果 |
|---|---|
| 路由/模块 | 无模块权限时不显示入口，直接访问返回 403 |
| 页面区域/面板 | 无敏感数据权限时隐藏整个区域或展示脱敏摘要 |
| 按钮/操作 | 无执行权限时隐藏操作或明确只读原因，后端再次校验 |
| 数据范围 | 根据专题、项目、负责人等授权过滤，不能仅靠前端筛选 |

同一任务、指标和状态在不同权限下保持同一事实值，禁止因“视角切换”构造两套互相矛盾的 Mock 或缓存。
