...
3. **写卡片（遵循 6 大原则）**：按需创建以下文件：
   ...
   - `wiki/code/` — 代码模块卡（type: code_module，必须绑定 `related_code`）
   - 🚨 **强约束**：行文必须符合【📝 知识编纂原则】。`pitfall`/`norm` 必须填 `affects_path`。`code_module` 必须填 `related_code`。
4. **双链焊死（CRITICAL！）**：
   - 知识绝不能是孤岛！你必须找到源头卡片并强制修改，追加反向引用。
   - 若已有卡片与新内容有支持或矛盾关系，用类型化标注：「✅ 支持」、「❌ 反驳」、「🔄 演化自」。
5. **黄页注册**：修改 `index.md`，将新卡片按分类追加到对应区域，附一行中文摘要。
6. **日志记账**：追加 `log.md`，格式为 `## [YYYY-MM-DD] ingest | 标题`，列出本次新建/更新的所有文件。
7. **重编译图索引**（⚡ 新增）：运行 `python scripts/compile_graph.py`，确认新卡片已正确连接到图谱。
